...
class DatabaseStep
{
    ...
    {
        // Get default values from environment variables (set by Synology SPK)
        $defaultDriver = env('DB_CONNECTION', 'sqlite');
        // Map 'mysql' driver to 'mariadb' for display purposes
        if ($defaultDriver === 'mysql') {
            $defaultDriver = 'mariadb';
        }
        $defaultHost = env('DB_HOST', '127.0.0.1');
        $defaultPort = env('DB_PORT', '3306');
        $defaultDatabase = env('DB_DATABASE', 'database.sqlite');
        $defaultUsername = env('DB_USERNAME', 'pelican');
                ...
                ToggleButtons::make('env_database.DB_CONNECTION')
                    ...
                    ->afterStateUpdated(function ($state, Set $set, Get $get) {
                        ...
                        switch ($state) {
                            ...
                            case 'mariadb':
                            case 'mysql':
                                $set('env_database.DB_HOST', $get('env_database.DB_HOST') ?? env('DB_HOST', '127.0.0.1'));
                                $set('env_database.DB_USERNAME', $get('env_database.DB_USERNAME') ?? env('DB_USERNAME', 'pelican'));
