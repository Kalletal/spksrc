...
service_postuninst()
{
    ...
    # Log all wizard variables for debugging
    echo "=== Pelican Uninstall $(date) ===" >> /tmp/pelican_uninstall.log
    echo "wizard_delete_data: '${wizard_delete_data}'" >> /tmp/pelican_uninstall.log
    echo "SYNOPKG_PKG_STATUS: '${SYNOPKG_PKG_STATUS}'" >> /tmp/pelican_uninstall.log
    env | grep -i wizard >> /tmp/pelican_uninstall.log 2>/dev/null || true
    env | grep -i SYNOPKG >> /tmp/pelican_uninstall.log 2>/dev/null || true

    # Delete data if user chose to
    # DSM passes "true" as string when checkbox is checked
    if [ "${wizard_delete_data}" = "true" ]; then
        echo "Condition matched: deleting data..." >> /tmp/pelican_uninstall.log
        # Remove Docker volumes
        docker volume rm pelican_panel_data 2>/dev/null || true
        docker volume rm pelican_panel_logs 2>/dev/null || true
        docker volume rm pelican-panel_data 2>/dev/null || true
        docker volume rm pelican-panel_logs 2>/dev/null || true
        # Remove data directories (both underscore and hyphen versions for compatibility)
        rm -rf "/var/packages/pelican_panel/var/data" 2>/dev/null || true
        rm -rf "/var/packages/pelican_panel/var" 2>/dev/null || true
        rm -rf "/var/packages/pelican-panel/var/data" 2>/dev/null || true
        rm -rf "/var/packages/pelican-panel/var" 2>/dev/null || true
        rm -rf "/volume1/@appdata/pelican_panel" 2>/dev/null || true
        rm -rf "/volume1/@appdata/pelican-panel" 2>/dev/null || true
        echo "Data deleted successfully" >> /tmp/pelican_uninstall.log
    else
        echo "Condition NOT matched, keeping data" >> /tmp/pelican_uninstall.log
    fi
}
